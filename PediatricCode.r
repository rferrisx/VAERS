...
# R 4.0.2 "Microsoft Open' or R 4.1.2 plus install latest data.table
...
# being nursed by vaccinated mothers. Some misplaced older (age) injuries. It does eliminate
...
all.vaers.data[data.table::between(AGE_YRS,5,11) &
	 data.table::between(CAGE_YR,5,11) &
	 mdy(VAX_DATE) > mdy('10/31/2021') &
	 !grepl("inappropriate age",All_symptoms) &
	 VAX_TYPE == "COVID19",.N,.(SEX,AGE=as.integer(AGE_YRS))][
	,dcast(.SD,AGE ~ SEX,value.var="N",fun.aggregate=fsum)]

all.vaers.data[data.table::between(AGE_YRS,5,11) &
	 data.table::between(CAGE_YR,5,11) &
	 mdy(VAX_DATE) > mdy('10/31/2021') &
	 !grepl("inappropriate age",All_symptoms) &
	 VAX_TYPE == "COVID19",.N,.(SEX,AGE=as.integer(AGE_YRS),VAX_MANU)][
	,dcast(.SD,AGE ~ VAX_MANU ,value.var="N",fun.aggregate=fsum)]


# For reference 12 - 18
all.vaers.data[data.table::between(AGE_YRS,12,18) &
	 data.table::between(CAGE_YR,12,18) &
	 mdy(VAX_DATE) > mdy('3/31/2021') &
	 !grepl("inappropriate age",All_symptoms) &
	 VAX_TYPE == "COVID19",.N,.(SEX,AGE=as.integer(AGE_YRS))][
	,dcast(.SD,AGE ~ SEX,value.var="N",fun.aggregate=fsum)]
